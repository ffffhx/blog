...
对 Git 来说，本地的 `C` 和 `D'` 都是远端没有的新提交。于是它会把这两个提交继续在远端分支顶部再重放一遍，最后得到：
...
最后，两条历史的共同祖先还是 `B`，于是别人的提交 `C` 也会顺着新的祖先关系出现在我的分支图里：
...
我最开始真的是这么干的。大概连续处理了六七次冲突，想着总能把历史理顺。结果每一次冲突解决后再次 rebase，都会继续制造新的“复制提交”，所以最后才会看到那么多重复 commit。
